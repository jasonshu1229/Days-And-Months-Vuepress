...
setTimeout的运行机制：执行该语句时，是立即把当前定时器代码交给`浏览器的其他线程`，当定时器在事件队列中满足设置的时间值时再将回调函数加入**任务队列**，之后的执行就交给任务队列负责。但是如果此时**任务队列**不为空，则需要等待，所以执行定时器内代码的时间可能会大于设置的时间，要等到执行栈内的主线程代码执行完毕再去执行。
